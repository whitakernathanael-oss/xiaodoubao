...
async function applyRuntime(theme: Theme, adapter: DoubaoAdapter, wallpaperDataUrl: string, extraCss: string): Promise<InjectionResult> {
  ...
  const wallpaperHost = (initial.appRoot[0] ?? initial.chatArea[0]) as HTMLElement;
...
html.doubao-skin .dbs-message-user { display: block !important; width: fit-content !important; min-width: min(280px, 72%) !important; max-width: min(72%, 760px) !important; overflow-wrap: anywhere !important; background: color-mix(in srgb, var(--dbs-contrast-base) var(--dbs-chat-safety-mix), var(--dbs-user-bubble)) !important; border: 1px solid var(--dbs-chat-border) !important; border-radius: var(--dbs-chat-radius) !important; box-shadow: 0 8px 28px rgb(0 0 0 / calc(var(--dbs-chat-shadow) * .35)) !important; }
